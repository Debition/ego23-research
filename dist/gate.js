...
(() => {
  ...
  function openContent(bundle) {
    ...
    const bootstrap = `<script>window.__EGO23_ASSET_URLS__=${safeMap};window.addEventListener('pageshow',e=>{if(e.persisted)location.reload();});document.addEventListener('DOMContentLoaded',()=>{const nav=document.querySelector('.topbar nav');if(nav){const a=document.createElement('a');a.id='lock-page';a.href=location.pathname;a.textContent='退出访问';a.addEventListener('click',e=>{e.preventDefault();document.querySelectorAll('video').forEach(v=>{v.pause();v.removeAttribute('src');v.removeAttribute('poster');v.load();});Object.values(window.__EGO23_ASSET_URLS__||{}).forEach(URL.revokeObjectURL);delete window.__EGO23_ASSET_URLS__;document.body.replaceChildren();location.replace(location.pathname);});nav.append(a);}});<\/script>`;
